...
dread <- read.csv(file='/Users/jenniferzhang/Desktop/Research with Will/covtype_tr.csv')
...
dread <- read.csv(file='/Users/jenniferzhang/Desktop/Research with Will/covtype_va.csv')
...
# dir.create("/Users/jenniferzhang/Desktop/Research with Will/HCL/results_covtype", recursive = TRUE)

tdfO <- as.data.frame(t(tauO.hat)) 
write.csv(tdfO, file = paste('/Users/jenniferzhang/Desktop/Research with Will/HCL/results_covtype/causal_forest_grf_test_set_results_O', '_numtrees', toString(num_trees), '_alpha', toString(p_alpha), '_min_node_size', toString(p_min_node_size), '_sample_fraction', toString(p_sample_fraction),'.csv', sep='')) 

print('predicting and writing to csv, cost') 
tdfC <- as.data.frame(t(tauC.hat)) 
write.csv(tdfC, file = paste('/Users/jenniferzhang/Desktop/Research with Will/HCL/results_covtype/causal_forest_grf_test_set_results_C', '_numtrees', toString(num_trees), '_alpha', toString(p_alpha), '_min_node_size', toString(p_min_node_size), '_sample_fraction', toString(p_sample_fraction),'.csv', sep='')) 
