...
class Experiment():
    ...
    def AUC_cpit_cost_curve_deciles_cohort_vis(self, pred_values, values, w, n9d_ni_usd, color, plot_random=False): 
        ...
        plt.plot(combined_series[:, 2], combined_series[:, 1], '-o', color = color, markersize=10, linewidth=3)
    ...
    def AUC_cpit_cost_curve_deciles_cohort_vis_with_propensity(self, pred_values, values, w, n9d_ni_usd, propensities, color, plot_random=False): 
        ## function plots the cost curve the slope of the curve correspond to 
        ## inverse of CPIT as a growth metric 
        ## [TODO] plan to compute Area Under Curve (AUC) 
        ##   pred_values: list of model predictions, same vertical dim as # data points 
        ...
        for idx in range(0, TD.shape[0], int(np.floor(TD.shape[0] / num_segments))): 
            if idx == 0: 
                ...
                treated_target_rpu = sum(1.0 * np.multiply(values[treated_targeted_filter], np.divide(e_hat, propensities[treated_targeted_filter]))) / sum(treated_targeted_filter) 
                treated_target_nipu = sum(1.0 * np.multiply(n9d_ni_usd[treated_targeted_filter], np.divide(e_hat, propensities[treated_targeted_filter]))) / sum(treated_targeted_filter) 
                ...
                untreated_untarget_rpu = sum(1.0 * np.multiply(values[untreated_untargeted_filter], np.divide(1-e_hat, propensities[untreated_untargeted_filter]))) / sum(untreated_untargeted_filter) 
                untreated_untarget_nipu = sum(1.0 * np.multiply(n9d_ni_usd[untreated_untargeted_filter], np.divide(1-e_hat, propensities[untreated_untargeted_filter]))) / sum(untreated_untargeted_filter) 
